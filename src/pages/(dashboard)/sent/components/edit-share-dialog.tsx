...
export const EditShareDialog = ({ open, onClose, initialData, onSubmit, isLoading }: EditShareDialogProps) => {
	...
	const handleSelectAll = () => {
		if (selectAll) {
			...
			const userIds = initialData.shareDocuments.map(doc => doc.toUser?.id).filter(Boolean) as string[]
	...
	const handleSubmit = async () => {
		...
		const updatedDoc = {
			...initialData,
			shareDocuments: initialData.shareDocuments?.filter(doc => 
				!doc.toUser?.id || !selectedUsers.includes(doc.toUser.id)
			) || []
		}
		...
		<Dialog open={open} onOpenChange={onClose}>
			<DialogContent className="max-w-md">
				<div className="flex items-center gap-2 p-4 border-b">
					...
					/>
					<label htmlFor="select-all" className="cursor-pointer text-base font-medium">
						اختيار الكل
					</label>
				</div>
				...
				<div className="space-y-4 max-h-96 overflow-y-auto py-2">
					{initialData?.shareDocuments?.map((doc, index) => (
						<div key={index} className="flex items-center gap-2 px-4 py-2 border-b">
							<Checkbox 
							...
							/>
							<label htmlFor={`user-${doc.toUser?.id}`} className="w-full cursor-pointer">
								{doc.toUser?.name}
							</label>
						</div>
					))}
